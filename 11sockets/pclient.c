...
int main(int argc, char const* argv[])
{
  ...
  // leggo i primi e calcolo la somma
  long tot=0;
  for(int i=0;i<n;i++) {
    e = readn(fd_skt,&tmp,sizeof(tmp));
    if(e!=sizeof(int)) termina("Errore read");
  ...
  // invio la somma, convertita in network order (big endian)
  tot = htobe64(tot);
  e = writen(fd_skt,&tot,sizeof(tot));
  if(e!=sizeof(tot)) termina("Errore write somma primi");
  if(close(fd_skt)<0)
