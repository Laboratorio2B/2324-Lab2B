...
import struct,socket,argparse,concurrent.futures,os,sys,time
...
def main(a,b,host,port):
  ...
  with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    ...
    s.connect((host, port))
    # print(f"{os.getpid()}: connesso a {s.getpeername()}")
    print(f"{os.getpid()}: connesso; richiede {a}-{b}")
    ...
    for p in primi:  
      print(f"{p:>12}",file=sys.stderr) # stampa p right aligned in 12 caratteri
    # per controllo restituisce al server la somma dei primi trovati in 64 bit
    s.sendall(struct.pack("!q",sum(primi)))  
    print(f"{os.getpid()}: somma inviata, termino la connessione")
