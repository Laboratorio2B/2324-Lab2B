  ...
  with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
    try:  
      ...
      while len(inputsk)>0:
        ...
        # dati da leggere o sono pronti per una mia scrittura 
        if len(inready) + len(outready) == 0:
          print("Sono in attesa")  # non ci sono socket pronti da leggere o scrivere
        else:
          for c in inready:
            if c is server:
              ...
              # tra i socket da monitorare in entrata (devono arrivare i 2 interi)
              inputsk.append(conn)
            elif c in attendisomma:
              # devo solo attendere la somma
              data = recv_all(c,8)
              somma = struct.unpack("!q",data)[0]
              print(f"Ricevuto somma {somma} da {c.getpeername()} comunicazione completata")
              attendisomma.remove(c)
              inputsk.remove(c)
              c.close()                  
            else:
      ...
      print("Input socket:",inputsk)
      print("Output socket:",outputsk)
